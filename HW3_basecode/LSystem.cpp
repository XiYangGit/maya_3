...
void LSystem::processPy(unsigned int n,
	...
	//pass values into python
	for (unsigned int i = 0; i < preBranches.size(); i++)
	{
		std::vector<float> branch_i;
		Branch preBranch_i = preBranches[i];
		vec3 startPt = preBranch_i.first;
		vec3 endPt = preBranch_i.second;
		branch_i.push_back(startPt[0]);
		branch_i.push_back(startPt[1]);
		branch_i.push_back(startPt[2]);
		branch_i.push_back(endPt[0]);
		branch_i.push_back(endPt[1]);
		branch_i.push_back(endPt[2]);
		branches.push_back(branch_i);
	}
	for (unsigned int i = 0; i < preFlowers.size(); i++)
	{
		std::vector<float> flower_i;
		Geometry preFlower_i = preFlowers[i];
		vec3 flowerPt = preFlower_i.first;
		flower_i.push_back(flowerPt[0]);
		flower_i.push_back(flowerPt[1]);
		flower_i.push_back(flowerPt[2]);
		flowers.push_back(flower_i);
	}
}
